...
import { MetadataEditor, jsonToPairs, pairsToJson, type MetaPair } from '../components/shared/MetadataEditor'
...
const KNOWN_FIELDS = ['name', 'institution', 'account_type', 'currency', 'is_active', 'notes', 'portfolio_id', 'id', 'created_at', 'updated_at']
...
export default function AccountsPage() {
  ...
  const createMut = useMutation({ mutationFn: createAccount, onSuccess: () => { qc.invalidateQueries({ queryKey: ['accounts'] }); closeModal() } })
  const updateMut = useMutation({ mutationFn: ({ id, ...rest }: Record<string, unknown>) => updateAccount(id as string, rest), onSuccess: () => { qc.invalidateQueries({ queryKey: ['accounts'] }); closeModal() } })
  const deleteMut = useMutation({ mutationFn: deleteAccount, onSuccess: () => qc.invalidateQueries({ queryKey: ['accounts'] }) })
  ...
  function openEdit(account: Account) {
    ...
    // Unknown fields from metadata
    const meta = (account as Record<string, unknown>).metadata as Record<string, unknown> | null
    setMetaPairs(jsonToPairs(meta))
    setErrors({})
    setShowImport(false)
    setModalOpen(true)
  }
  ...
  function handleImportJson() {
    try {
      ...
      for (const [k, v] of Object.entries(parsed)) {
        if (KNOWN_FIELDS.includes(k)) known[k] = v
        else extra[k] = v
      }
      setForm((f) => ({ ...f, ...(known as Partial<typeof form>) }))
      setMetaPairs(jsonToPairs(extra as Record<string, unknown>))
      setImportError('')
      setShowImport(false)
      setImportText('')
    } catch {
  ...
  function handleSubmit() {
    ...
    const payload = { ...form, ...(Object.keys(metadata).length ? { metadata } : {}) }
    if (editTarget) {
  ...
  function handleDelete() {
    ...
    deleteMut.mutate(deleteTarget.id, {
      onError: (err: unknown) => {
        ...
        setDeleteError(msg ?? 'Cannot delete — this account may have linked assets or transactions')
      },
  ...
  return (
    <div className="p-6 pb-20 md:pb-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-[#0d9488] hover:bg-teal-700 text-white text-[13px] font-medium rounded-lg transition-colors">
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Account</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Type</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Institution</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Currency</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Positions</th>
            ...
            <tbody>
              {accounts.map((acc) => (
                <tr key={acc.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className="font-mono text-gray-600">{acc.currency}</span>
                  </td>
                  <td className="px-4 py-3">
                    <PositionCountBadge count={acc.position_count} />
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      <ActionBtn
                        ...
                        icon="delete"
                        title={acc.position_count > 0 ? `Cannot delete — ${acc.position_count} linked asset${acc.position_count !== 1 ? 's' : ''}` : 'Delete'}
                        danger
      ...
      <Modal
        open={modalOpen}
        onClose={closeModal}
        title={editTarget ? `Edit — ${editTarget.name}` : 'Add account'}
        width="max-w-lg"
      >
        <div className="space-y-5">
            ...
            {showImport && (
              <div className="mt-2 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { setShowImport(false); setImportText(''); setImportError('') }} className="px-3 py-1.5 text-[12px] font-medium text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">Cancel</button>
                </div>
              </div>
            )}
          </div>
          ...
          <FormSection title="Basic details">
            <Field label="Account name" required error={errors.name}>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. IBI Broker" error={!!errors.name} />
            </Field>
            <FormGrid>
            ...
            <Field label="Institution / provider">
              <Input value={form.institution} onChange={(e) => setForm({ ...form, institution: e.target.value })} placeholder="e.g. IBI Bank, Interactive Brokers" />
            </Field>
          </FormSection>
          ...
          <FormSection title="Settings">
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              />
              <span className="text-[13px] text-gray-700 font-medium">Active — include in portfolio calculations</span>
            </label>
          </FormSection>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <div className="flex gap-2">
              <button onClick={closeModal} className="px-4 py-2 text-[13px] font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">
                Cancel
              </button>
              <button
...
function PositionCountBadge({ count }: { count: number }) {
  return (
    <span className={clsx(
      'text-[11px] px-2 py-0.5 rounded font-semibold tabular-nums',
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={clsx('text-[11px] px-2 py-0.5 rounded font-semibold', active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-400')}>
      {status || '—'}
    </span>
  )
}
